				...
				<tbody>
					<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
								    ...
								    <span>认购数量：<?php echo ($vo["num"]); ?></span>
								</td>
								<td>
								    <span>支付币种：<?php echo strtoupper($vo['buycoin']);?></span><br />
								    <span>认购数量：<?php echo ($vo["mum"]); ?></span>
								</td>
								<td>
								    <span>认购日期：<?php echo ($vo["addtime"]); ?><br />
								    <span>解冻时间：<?php echo ($vo["endtime"]); ?></span>
								</td>
								
								<td>
								    <?php if(($vo["status"]) == "1"): ?><span style="color:red;">冻结中</span>
								    <?php else: ?>
								    <span style="color:green;">已解冻</span><?php endif; ?>
								</td>
							</tr><?php endforeach; endif; else: echo "" ;endif; ?>
					<?php else: ?>
						<td colspan="15" class="text-center empty-info"><i class="glyphicon glyphicon-exclamation-sign"></i>暂无数据</td><?php endif; ?>
				</tbody>
			</table>
			<div class="page">
