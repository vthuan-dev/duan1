...
<head>
	...
	<link rel="stylesheet" href="/jiaoyi-project/xhj/Public/Static/css/kline.css">
	<style>
		#chart_container {
			margin: 0 auto;
			width: 100%;
...
<div id="chart_container">
    <div class="loding" data-type="0">
        <img src="/jiaoyi-project/xhj/Public/Static/img/loading.gif" alt="" />
    </div>
</div>
<div id="chart_container1" style="display:none"></div>
</body>
<script>
	// Định nghĩa hàm loading nếu chưa có
	function loading() {
...
<script>
	function initOnReady() {
	    console.log('Starting initOnReady...');
	    
	    // Kiểm tra các thư viện cần thiết
	    if (typeof VyKline === 'undefined') {
	        console.error('VyKline is not loaded');
	        return;
	    }
	    
	    if (typeof window.deedfeeds === 'undefined') {
	        console.error('window.deedfeeds is not defined');
	        return;
	    }
	    
	    // Kiểm tra container
	    const container = document.getElementById('chart_container1');
	    if (!container) {
	        console.error('chart_container1 not found');
	        return;
	    }
	    
	    console.log('All checks passed, initializing kline...');
		...
		try {
		    ...
		    console.log('VyKline.bootstrap completed');
		    
		    // Kiểm tra vyChart trước khi sử dụng
		    if (typeof vyChart !== 'undefined') {
		        vyChart.switchChartType('KLINE')
		        console.log('vyChart.switchChartType completed');
		    }
		    
		    console.info('window.deedfeeds:', window.deedfeeds)
		} catch (error) {
		    console.error('Error initializing kline chart:', error);
		}
	}
	window.addEventListener('DOMContentLoaded', function() {
	    // Delay để đảm bảo tất cả thư viện đã load
